...
class HipReturnsConnectorSpec extends PlaySpec with GuiceOneServerPerSuite with MockitoSugar with BeforeAndAfter {
  ...
  trait Setup extends ConnectorTest {
    ...
    val connector = new TestHipReturnsConnector()
  }


  "HipReturnsConnector" must {
    ...
    "submit ated returns" must {
      "Correctly Submit a return with reliefs" in new Setup {
        ...
        when(requestBuilderExecute[HttpResponse]).thenReturn(Future.successful(HttpResponse(201, wrappedSuccessResponse, Map.empty[String, Seq[String]])))
      ...
      "submit ated returns with unprocessable entity response converted to relevant status" in new Setup {
    ...
    "get summary returns" must {
      ...
      "not return data if we get some other status" in new Setup {
        ...
        when(requestBuilderExecute[HttpResponse]).thenReturn(Future.successful(HttpResponse(BAD_REQUEST, wrappedSuccessResponse, Map.empty[String, Seq[String]])))
    ...
    "get form bundle returns" must {
      ...
      "get form bundle with unprocessable entity response converted to relevant status" in new Setup {
    ...
    "submit edited liability returns" must {
      ...
      "submit edited liability returns with unprocessable entity response converted to relevant status" in new Setup {
