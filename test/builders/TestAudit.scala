...
class TestAudit(val auditConnector: AuditConnector) extends Audit("test", auditConnector) {
  ...
  override def as[A](auditMagnet: AuditAsMagnet[A])(body: Body[A])(using hc: HeaderCarrier, ec: ExecutionContext): A = {
  ...
  override def sendDataEvent(de: DataEvent)(using ec: ExecutionContext): Unit = captureDataEvent(de)
}
