...
object FeatureSwitch {
  ...
  def apply(name: String, @annotation.unused enabled: Boolean = false)(using config: ServicesConfig): FeatureSwitch = getProperty(name)
  def unapply(fs: FeatureSwitch): Option[(String, Boolean)] = Some(fs.name -> fs.enabled)
}

object ATEDFeatureSwitches extends ATEDFeatureSwitches
